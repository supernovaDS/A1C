...
export default function PptToPdf() {
  ...
  const convertPpt = async () => {
    ...
    try {
      ...
      // Extract extension (ppt or pptx) to dynamically route the API
      const fileExt = file.name.split('.').pop().toLowerCase();
      if (fileExt !== 'ppt' && fileExt !== 'pptx') {
        throw new Error("Invalid file type. Please upload a PPT or PPTX file.");
      }

      const formData = new FormData();
      formData.append('File', file);

      // Dynamically use /ppt/to/pdf or /pptx/to/pdf
      ...
      if (!response.ok) {
        throw new Error(data.Message || "Failed to convert presentation.");
      }
    ...
    <div className="max-w-2xl mx-auto mt-10 p-4 sm:p-6 bg-white rounded-2xl shadow-sm border border-gray-100">
      ...
      {!resultUrl ? (
          ...
          {file && (
            <div className="mt-6 flex flex-col sm:flex-row items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 gap-4">
              ...
              <button 
                ...
                className="w-full sm:w-auto bg-orange-600 hover:bg-orange-700 text-white px-6 py-3 rounded-lg font-bold transition-colors flex items-center justify-center gap-2 disabled:opacity-70 shrink-0"
              >
                {isConverting ? <Loader2 className="animate-spin w-5 h-5" /> : 'Convert to PDF'}
              </button>
            </div>
          )}
        </>
        ...
        <div className="text-center py-10 px-4 bg-green-50 rounded-xl border border-green-200">
          <h3 className="text-xl font-bold text-green-700 mb-4">Slides Converted!</h3>
          <p className="text-green-600 mb-6">Your presentation has been perfectly formatted into a PDF.</p>
